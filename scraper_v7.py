...
async def buscar_carrera(page, termino, carrera_sies, idx):
    avisos = []
    print(f"\n  [{idx}/{len(CARRERAS)}] '{termino}'")
...
async def main():
    print("\n" + "="*60)
    print("  MONITOR MERCADO LABORAL — AGROPECUARIA v6")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"  Flag genérico: > {UMBRAL_AVISO_GENERICO} carreras requeridas")
    print("="*60)
    ...
    async with async_playwright() as pw:
        ...
        for area, carreras in CARRERAS_POR_AREA.items():
            ...
            for termino, carrera_sies in carreras.items():
                idx_global += 1
                avisos = await buscar_carrera(page, termino, carrera_sies, idx_global)
                for a in avisos:
                    id_ = a['idOferta']
