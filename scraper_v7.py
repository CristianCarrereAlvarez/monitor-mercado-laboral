...
import asyncio, csv, json, re, random, time, sys
...
# ══════════════════════════════════════════════════════════
# CONFIGURACIÓN
# El área se pasa como argumento al correr el script:
#   python scraper_v7.py Agropecuaria
#   python scraper_v7.py Salud
#   python scraper_v7.py Educación
# Si no se pasa argumento, usa "Agropecuaria" por defecto.
# ══════════════════════════════════════════════════════════

AREAS_VALIDAS = list(CARRERAS_POR_AREA.keys())

if len(sys.argv) > 1:
    AREA = sys.argv[1]
    if AREA not in AREAS_VALIDAS:
        print(f"✗ Área '{AREA}' no existe.")
        print(f"  Áreas válidas: {', '.join(AREAS_VALIDAS)}")
        sys.exit(1)
else:
    AREA = "Agropecuaria"   # default

CARRERAS = CARRERAS_POR_AREA[AREA]

PAUSA_ENTRE_CALLS     = (1.0, 2.5)
UMBRAL_AVISO_GENERICO = 20

TIMESTAMP = datetime.now().strftime('%Y_%m')
slug_area = AREA.lower().replace(' ', '_').replace('é','e').replace('ó','o').replace('ó','o')
CSV_FILE  = f"{slug_area}_{TIMESTAMP}.csv"
JSON_FILE = f"{slug_area}_{TIMESTAMP}.json"
...
async def buscar_carrera(page, termino, carrera_sies, idx, total=0):
    ...
    for pagina in range(1, 9999):   # termina cuando el API dice que no hay más
        try:
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        aviso_datos    = {}   # aviso_id → datos del listado
        aviso_detalles = {}   # aviso_id → JSON del detalle (1 fetch por aviso único)
        aviso_carreras = {}   # aviso_id → set de carrera_sies que lo encontraron

        sies_a_termino = {sies: term for term, sies in CARRERAS.items()}

        # ════════════════════════════════════════════════════
        # FASE 1 — Búsqueda: todas las carreras del área
        # ════════════════════════════════════════════════════
        n_carreras = len(CARRERAS)
        print(f"\n{'='*60}")
        print(f"  FASE 1: Búsqueda — {AREA}")
        print(f"  {n_carreras} carreras — todas las páginas disponibles")
        print(f"{'='*60}")

        for idx, (termino, carrera_sies) in enumerate(CARRERAS.items(), 1):
            avisos = await buscar_carrera(page, termino, carrera_sies, idx, n_carreras)
            for a in avisos:
                id_ = a['idOferta']
                if id_ not in aviso_datos:
                    aviso_datos[id_] = a
                aviso_carreras.setdefault(id_, set()).add(carrera_sies)
            esperar()

        n_unicos = len(aviso_datos)
        n_pares  = sum(len(c) for c in aviso_carreras.values())
        print(f"\n  Avisos únicos:       {n_unicos}")
        print(f"  Pares aviso×carrera: {n_pares}")

        # ════════════════════════════════════════════════════
        # FASE 2 — Detalle: 1 sola llamada por aviso único
        # ════════════════════════════════════════════════════
        print(f"\n{'='*60}")
    ...
    # ════════════════════════════════════════════════════
    # FASE 3 — Expandir: 1 fila por (aviso × carrera)
    # ════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    ...
    print(f"  Filas: {len(resultados)}  ({n_unicos} avisos × avg {len(resultados)/max(n_unicos,1):.1f} carreras)")
    ...
    print(f"\n  Por carrera genérica SIES:")
    from collections import Counter
    for carrera, cnt in Counter(r['carrera_generica_sies'] for r in resultados).most_common():
        gen = sum(1 for r in resultados if r['carrera_generica_sies']==carrera and r.get('flag_aviso_generico'))
        print(f"    {cnt:5d} ({gen:3d} gen.)  {carrera}")
