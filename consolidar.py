...
COLS_AVISOS = [
    ...
    'fecha_desactivacion_detectada', 'dias_publicado_hasta_baja', 'censurado',
    # contenido
...
COLS_CARRERAS = [
    'carrera_trabajando', 'n_avisos_acum',
...
def main(dir_crudo, dir_maestras):
    ...
    for nombre, m in car_meta.items():
        ...
        filas_car.append(m)
    filas_car.sort(key=lambda r: -r['n_avisos_acum'])
    ...
    bajas = [a for a in vals if not a['censurado']]
    dur   = sorted(a['dias_publicado_hasta_baja'] for a in bajas
    ...
    if dur:
        ...
        print(f"    días publicado→baja  : sin datos "
              f"(se necesita más de una corrida)")

    print(f"\n  TAXONOMÍA OBSERVADA")
    print(f"    carreras distintas     : {len(filas_car)}")
    print(f"    instituciones distintas: {len(filas_inst)}")
    if filas_car:
        tot = sum(r['n_avisos_acum'] for r in filas_car)
        for n in (50, 100, 200):
            if len(filas_car) > n:
                acum = sum(r['n_avisos_acum'] for r in filas_car[:n])
